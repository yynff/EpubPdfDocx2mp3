...
def start_conversion(file_path, custom_out_path, log_widget, metrics_label, start_btn, stop_btn):
    ...
    # 🎯 核心黑科技：智能动态路径适配
    if custom_out_path:
        # 如果老用户手动指定了路径，百分之百听从指挥
        final_output_dir = custom_out_path
    else:
        # 如果用户没指定，自动执行方向 A：拿文件名动态创建专属文件夹（专书专包）
...
def create_gui():
    root = tk.Tk()
    root.title("全能外刊 AI 语音转换控制台 v3.0 旗舰情怀版")
    ...
    log_text.insert(tk.END, "💡 旗舰情怀版使用向导 (v3.0)：\n* 迎合两代人习惯设计：若【自定义路径】留空，系统自动按书名创建独立文件夹（年轻人最爱）。\n* 若点击紫色按钮指定了固定目录（如同步盘），所有 MP3 将绝对听从您的指挥，统一集中归档（Windows 老将风范）！\n------------------------------------------------------------\n")
    root.mainloop()
