...
def scan_and_extract_docx(docx_path, log_widget):
    log_widget.insert(tk.END, "📝 正在启动 [Word] 物理段落智能分析流程...\n")
    log_widget.update()
    chapters = []
    try:
		...
		"text": full_text,
                "title": f"Word文章: {raw_title}" 
		
            })
    except Exception as e:
        log_widget.insert(tk.END, f"❌ 读取 Word 文件出错: {e}\n")
    return chapters
...
def start_conversion(file_path, custom_out_path, log_widget, metrics_label, start_btn, stop_btn):
    ...
    if custom_out_path:
        ...
        final_output_dir = os.path.join(base_dir, f"[音频]_{sanitize_filename(file_name_only)}")
...
def create_gui():
    root = tk.Tk()
    root.title("全能外刊 AI 语音转换控制台 v3.5 旗舰情怀升级版")
    ...
    tk.Label(root, text="第一栏：输入/选择文档路径 (支持 .epub / .pdf / .docx)", font=("Segoe UI", 10, "bold"), anchor="w").pack(fill=tk.X, padx=15, pady=(15, 2))
    ...
    def select_file():
        fp = filedialog.askopenfilename(title="选择外刊文件", filetypes=[("Supported Files", "*.epub;*.pdf;*.docx")])
    ...
    tk.Label(root, text="新增栏：自定义音频输出目录 (留空则默认执行年轻人的【动态命名专书专包】)", font=("Segoe UI", 10, "bold"), anchor="w").pack(fill=tk.X, padx=15, pady=(5, 2))
    ...
    def select_output_dir():
        dp = filedialog.askdirectory(title="选择音频保存文件夹")
        if dp: custom_output_path.set(dp)
        
    tk.Button(out_frame, text="自定义路径", command=select_output_dir, bg="#9C27B0", fg="white", font=("Segoe UI", 9, "bold"), padx=10).pack(side=tk.RIGHT)
    
    tk.Label(root, text="第二栏：时延与云端流量消费监控 (Metrics)", font=("Segoe UI", 10, "bold"), anchor="w").pack(fill=tk.X, padx=15, pady=(5, 2))
    ...
    log_text.insert(tk.END, "💡 旗舰情怀版使用向导 (v3.5)：\n* 若【自定义路径】留空，系统自动按书名创建独立文件夹\n------------------------------------------------------------\n")
    root.mainloop()
